...
const operations: Record<string, Partial<Record<Contender, Factory>>> = {
	forEach: {
		...
		'neotraverse v1': (d) => () =>
			v1.forEach(d, (_c: any, n: any) => {
				if (typeof n === 'number') sink += n;
			}),
		'neotraverse/safe': (d) => () => {
			for (const node of v2.visit(d))
				if (typeof node.value === 'number') sink += node.value as number;
		},
	},
	map: {
		traverse: (d) => () =>
	...
	reduce: {
		...
		'neotraverse/safe': (d) => () => {
			let n = 0;
			for (const node of v2.visit(d)) if (node.isLeaf) n++;
			sink += n;
		},
	},
	paths: {
		...
		'neotraverse v1': (d) => () => {
			sink += v1.paths(d).length;
		},
		'neotraverse/safe': (d) => () => {
			let n = 0;
			for (const node of v2.visit(d)) {
				sink += node.path.length;
				n++;
			}
			sink += n;
		},
	...
	nodes: {
		...
		'neotraverse v1': (d) => () => {
			sink += v1.nodes(d).length;
		},
		'neotraverse/safe': (d) => () => {
			let n = 0;
			for (const _ of v2.visit(d)) n++;
			sink += n;
		},
	},
};
...
const path_operations: Record<string, Partial<Record<Contender, Factory>>> = {
	get: {
		...
		'neotraverse v1': (d) => () => {
			sink += v1.get(d, DEEP_PATH);
		},
		'neotraverse/safe': (d) => () => {
			sink += v2.get(d, DEEP_PATH);
		},
	},
	has: {
		traverse: (d) => () => {
		...
		'neotraverse v1': (d) => () => {
			sink += v1.has(d, DEEP_PATH) ? 1 : 0;
		},
		'neotraverse/safe': (d) => () => {
			sink += v2.has(d, DEEP_PATH) ? 1 : 0;
		},
	},
	set: {
		...
		traverse: (d) => () => {
			sink += traverseOrig(d).set(DEEP_PATH, 1) ? 1 : 0;
		},
		'neotraverse v1': (d) => () => {
			sink += v1.set(d, DEEP_PATH, 1) ? 1 : 0;
		},
		'neotraverse/safe': (d) => () => {
			sink += v2.set(d, DEEP_PATH, 1, { mutate: true }) ? 1 : 0;
		},
	},
};
...
async function run_suite(
	operation: string,
	dataset: string,
	factories: Partial<Record<Contender, Factory>>,
) {
...
async function run_v2_only(label: string, fn: () => void) {
	...
	console.log(
		`  ${label.padEnd(28)} ${out.opsPerSec.toLocaleString('en-US').padStart(12)} ops/s  ±${out.rme}%`,
	);
	return out;
}
...
async function main() {
	...
	const v2only: any[] = [];
	v2only.push(
		await run_v2_only('transform · no-op identity (2000)', () => {
			sink += v2.transform(big, () => {}) === big ? 1 : 0;
		}),
	);
	v2only.push(
		await run_v2_only('transform · single sparse edit (2000)', () => {
			sink += v2.transform(big, (n: any, e: any) =>
				n.key === 'name' && n.value === 'item-1000' ? e.replace('x') : undefined,
			)
				? 1
				: 0;
		}),
	...
	v2only.push(
		await run_v2_only('merge · json', () => {
			sink += v2.merge(realistic, { profile: { age: 42 } }) ? 1 : 0;
		}),
	);
	v2only.push(
		await run_v2_only('diff · array(2000) one change', () => {
			sink += v2.diff(big, bigB).length;
		}),
	);
	v2only.push(
		await run_v2_only('equal · json', () => {
			sink += v2.equal(realistic, structuredClone(realistic)) ? 1 : 0;
		}),
	);
	v2only.push(
		await run_v2_only('resolveRefs · refs', () => {
			sink += v2.resolveRefs(refDoc) ? 1 : 0;
		}),
	);
	v2only.push(
		await run_v2_only('visit match · posts.*.id', () => {
			sink += v2.visit(realistic, 'posts.*.id').toArray().length;
		}),
	);
	...
	const summary = {
		metric: 'geometric-mean v2 throughput ÷ v1 on equivalent ops',
		v2VsV1Geomean: +geomean(ratios).toFixed(2),
	};
