...
> The hardening and DoS guards apply to the **functional** API. The `neotraverse/legacy`
> drop-in is intentionally kept behaviour-compatible with the original `traverse` and does
> **not** receive them — run untrusted data through the functional API.
