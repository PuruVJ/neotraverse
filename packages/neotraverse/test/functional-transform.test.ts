...
describe('deleteWhere / prune on arrays (C-3)', () => {
	test('deleteWhere removes every matching array element (no skipping)', () => {
		expect(deleteWhere([1, 2, 3, 4, 5, 6], (_, x) => typeof x === 'number' && x % 2 === 0)).toEqual(
			[1, 3, 5],
		);
	});
	...
	test('prune keeps only matching array elements', () => {
		expect(
			prune([1, 2, 3, 4], (ctx, x) => ctx.isRoot || (typeof x === 'number' && x % 2 === 1)),
		).toEqual([1, 3]);
	});
...
describe('groupBy', () => {
	test('buckets visited values by key', () => {
		const g = groupBy({ a: 1, b: 2, c: 3, d: 4 }, (ctx, v) =>
			ctx.isLeaf ? (v % 2 ? 'odd' : 'even') : 'branch',
		);
