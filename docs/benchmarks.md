...
[`neotraverse/safe`](/guide/safe) is a separate, opt-in core built on an **iterative** engine. It is **not** a faster successor — it trades a little throughput for **stack-safety** and **bounded memory on partial consumption**. The point of these numbers is *when to reach for it*, not "/safe is faster".
